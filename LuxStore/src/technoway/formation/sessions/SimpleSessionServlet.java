...
public class SimpleSessionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<BODY><H1>Example session servlet to demostrate session tracking and life cycle</H1>");
		...
		out.println("Max in active time interval: " + session.getMaxInactiveInterval() + "</BR>");
		// Checks whether the requested session ID came in as a cookie
		out.println("Session ID came in as a cookie: " + request.isRequestedSessionIdFromCookie() + "</BR>");

		out.println("<H2>Iteratively printing all the values associated with the session:</H2>");
